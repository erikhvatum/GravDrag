...
final class MetalRenderer: NSObject, MTKViewDelegate {
    ...
    // UI overlay buffers (selection rect / lasso)
    private var uiLineBuffer:  MTLBuffer!  // NDC vertices for lines
    private var uiRectBuffer:  MTLBuffer!  // NDC vertices for filled semi-transparent rect
    private let kMaxUIVerts = 1024
    ...
    init(device: MTLDevice, pixelFormat: MTLPixelFormat) throws {
        ...
        super.init()
        try buildPipelines(pixelFormat: pixelFormat)
        uiLineBuffer = device.makeBuffer(
            length: MemoryLayout<SIMD2<Float>>.stride * kMaxUIVerts,
            options: .storageModeShared)!
        uiRectBuffer = device.makeBuffer(
    ...
    func draw(in view: MTKView) {
        ...
        // ── Draw each body (filled polygon via triangle fan) ──────────────
        ...
        for (i, body) in sim.bodies.enumerated() {
            var idx = Int32(i)
            encoder.setVertexBytes(&idx, length: MemoryLayout<Int32>.size, index: 3)
            // Triangle fan: vertex 0 is implicit center (not stored); Metal doesn't
            // support fan natively in MPS. We use a pre-built triangle-list stored
            // in an index buffer OR we do a fan draw via sequential triangles.
            // We triangulate each body CPU-side and draw as triangle list.
            let tris  = body.triangleIndices
            let count = tris.count * 3
            if count >= 3 {
                // Write index data to a small shared buffer
                let ibuf = device.makeBuffer(
                    bytes: tris.flatMap { [$0.0, $0.1, $0.2] }.map { UInt32($0) },
        ...
        for (i, body) in sim.bodies.enumerated() where body.isSelected {
            var idx = Int32(i)
            encoder.setVertexBytes(&idx, length: MemoryLayout<Int32>.size, index: 3)
            // Draw N+1 vertices so the strip closes (last = first)
            let count = body.localVertices.count + 1
            encoder.drawPrimitives(type: .lineStrip, vertexStart: 0, vertexCount: count)
        }
    ...
    private func drawOverlay(encoder: inout MTLRenderCommandEncoder, uniforms: RenderUniforms) {
        guard selectionOverlay.isActive else { return }
        encoder.setRenderPipelineState(uiRenderPipeline)
        ...
        case .rect(let r):
            // Convert from view/world space CGRect to NDC
            let tl = worldToNDC(
                SIMD2<Float>(Float(r.minX), Float(r.maxY)), uniforms: uniforms, ar: ar)
            let br = worldToNDC(
                SIMD2<Float>(Float(r.maxX), Float(r.minY)), uniforms: uniforms, ar: ar)
            // Filled semi-transparent rectangle (2 triangles)
            let filled: [SIMD2<Float>] = [
                SIMD2(tl.x, tl.y), SIMD2(br.x, tl.y), SIMD2(br.x, br.y),
                SIMD2(tl.x, tl.y), SIMD2(br.x, br.y), SIMD2(tl.x, br.y)
...
extension Body {
    ...
    private func buildTriangles() -> [(Int, Int, Int)] {
        let idxFlat = earClipTriangulate(localVertices)
        var result: [(Int, Int, Int)] = []
        stride(from: 0, to: idxFlat.count - 2, by: 3).forEach { k in
            result.append((idxFlat[k], idxFlat[k+1], idxFlat[k+2]))
        }
        return result
    }
